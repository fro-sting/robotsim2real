...
if do_concatenate:
    ...
    # 存储每个文件的数据和关键帧信息
    all_partition_data = []
    key_frames_info = []  # [(start_frame, end_frame), ...]
    
    for file_idx, file_path in enumerate(partition_vicon_files):
        
        data = np.load(file_path, allow_pickle=True)
        all_partition_data.append(dict(data))
        
        # 计算关键帧（以joint 2为例，也可以选择其他关节）
        joint_data = data['jointpos'][:, 2]  # 使用joint 2作为参考
        # 前200帧最大值作为起始帧
        front_200_max_idx = np.argmax(joint_data[:200])
        # 后150帧最大值作为结束帧
        back_150_max_idx = np.argmax(joint_data[-150:]) + len(joint_data) - 150
        key_frames_info.append((front_200_max_idx, back_150_max_idx))
        print(f"文件 {file_idx+1}: 起始帧={front_200_max_idx}, 结束帧={back_150_max_idx}, 总长度={len(joint_data)}")
    
    # 拼接所有数据
    concatenated_data = {}
    # 获取所有key
    all_keys = list(all_partition_data[0].keys())
    for key in all_keys:
        concatenated_arrays = []
        for i, partition_data in enumerate(all_partition_data):
            if key in partition_data:
                start_frame, end_frame = key_frames_info[i]
                # 提取从起始帧到结束帧的数据
                if partition_data[key].ndim == 2:
                    segment_data = partition_data[key][start_frame:end_frame+1, :]
                else:
                    segment_data = partition_data[key][start_frame:end_frame+1]
                # 对数据进行平滑化处理
                if segment_data.ndim == 2:
                    # 2维数据：对每一列分别进行平滑化
                    smoothed_data = np.zeros_like(segment_data)
                    for col in range(segment_data.shape[1]):
                        smoothed_data[:, col] = smooth_data(segment_data[:, col])
                else:
                    # 1维数据：直接平滑化
                    smoothed_data = smooth_data(segment_data)
                
                concatenated_arrays.append(smoothed_data)
        # 拼接所有段
        if concatenated_arrays:
            if concatenated_arrays[0].ndim == 2:
                # 2维数据拼接
                final_concatenated = []
                for i, segment in enumerate(concatenated_arrays):
                    if i == 0:
                        # 第一段直接添加
                        final_concatenated.append(segment)
                    else:
                        # 后续段需要与前一段末尾对齐
                        if key == 'root_pos':
                            # 对于root_pos，对第一列和第二列（x和y方向）进行位置对齐
                            prev_end_x = final_concatenated[-1][-1, 0]  # 前一段末尾的x位置
                            prev_end_y = final_concatenated[-1][-1, 1]  # 前一段末尾的y位置
                            current_start_x = segment[0, 0]  # 当前段开始的x位置
                            current_start_y = segment[0, 1]  # 当前段开始的y位置
                            x_offset = prev_end_x - current_start_x
                            y_offset = prev_end_y - current_start_y
                            
                            # 对整个段应用x和y方向偏移
                            adjusted_segment = segment.copy()
                            adjusted_segment[:, 0] += x_offset
                            adjusted_segment[:, 1] += y_offset
                            
                            final_concatenated.append(adjusted_segment)
                        else:
                            # 其他数据正常添加
                            final_concatenated.append(segment)
                
                concatenated_data[key] = np.vstack(final_concatenated)
            else:
                # 1维数据拼接
                concatenated_data[key] = np.concatenate(concatenated_arrays)
    
    # 保存拼接后的数据
    save_path = "/home/zl/Downloads/kuavo-rl-train-run/RL_train/output_data/vicon/concatenated_vicon_data.npz"
    ...
    if 'jointpos' in concatenated_data:
        ...
        for idx, joint in enumerate(joint_indices):
            ...
            # 标记每个段的边界
            current_pos = 0
            for i, (start_frame, end_frame) in enumerate(key_frames_info):
                segment_length = end_frame - start_frame + 1
                ax.axvline(x=current_pos, color='red', linestyle='--', alpha=0.7, 
                          label=f'segment_{i+1}_start' if i == 0 else "")
                ax.axvline(x=current_pos + segment_length - 1, color='blue', linestyle='--', alpha=0.7,
                          label=f'segment_{i+1}_end' if i == 0 else "")
                current_pos += segment_length
        
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.subplots_adjust(bottom=0.15)
        plt.suptitle("Vicon Joint Position after concatenation", fontsize=16)
        plt.show()
    ...
    if 'root_pos' in concatenated_data:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.subplots_adjust(bottom=0.15)
        plt.suptitle("Vicon Root Position after concatenation", fontsize=16)
        plt.show()
    ...
    if 'root_vel_world' in concatenated_data:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.subplots_adjust(bottom=0.15)
        plt.suptitle("Vicon Root Velocity (World Frame) after concatenation", fontsize=16)
        plt.show()
...
if do_align_with_ref:
    ...
    # 加载拼接后的数据
    concatenated_path = "/home/zl/Downloads/kuavo-rl-train-run/RL_train/output_data/vicon/concatenated_vicon_data.npz"
