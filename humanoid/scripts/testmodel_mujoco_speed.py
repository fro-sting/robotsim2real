...
class Sim2RealCMAOptimizer:
    ...
    def get_current_command(self, step, total_steps, target_command, use_gradual=True):
        """计算当前步的command值 - 前一半线性加速，后一半稳定"""
        if not use_gradual:
            return target_command
        
        half_steps = total_steps // 2  # 前一半步数用于加速
        
        if step < half_steps:
            # 前一半：线性加速从0到目标速度
            progress = step / half_steps  # 0 到 1
        ...
        current_command = [
            target_command[0] * scale,  # vx线性增长然后稳定
            target_command[1],          # vy保持不变
            target_command[2],          # dyaw保持不变  
            target_command[3]           # stand保持不变
        ]
        
        return current_command

    def save_and_visualize_data(self, real_data, sim_data, params=None, command=None, use_gradual_acceleration=True, total_steps=1000):
   
        # 🔥 生成与数据长度匹配的command序列
        if command is not None and use_gradual_acceleration:
            # 使用数据切片后的长度生成command
            data_length = len(real_data)  # 与数据处理逻辑保持一致
            command_sequence = []
            ...
            for step in range(data_length):
                ...
                current_cmd = self.get_current_command(
                    ...
                    data_length, 
                    command, 
                    use_gradual_acceleration
                )
                command_sequence.append(current_cmd)
            
            command_sequence = np.array(command_sequence)
            print(f"生成渐进command序列，长度: {len(command_sequence)}, 范围: vx从{command_sequence[0][0]:.3f}到{command_sequence[-1][0]:.3f}")
        else:
            command_sequence = command  # 使用原始固定command
        ...
        plotter._plot_velocity_comparison(base_lin_vel_real, base_lin_vel_sim, vel_names,
                                ...
                                command=command_sequence)  # 🔥 传入渐进command序列
        print("生成仅仿真速度对比图...")
        empty_vel_real = np.zeros((0, 2))  # 创建空的真实速度数据
        plotter._plot_velocity_comparison(empty_vel_real, base_lin_vel_sim, vel_names,
                                'Base Linear Velocity (Sim Only)', 'base_linear_velocity_sim_only.png', 
                                command=command_sequence)  # 🔥 传入渐进command序列
    ...
    def collect_mujoco_data(self, steps=1000, command=[2.0, 0., 0., 0.], use_gradual_acceleration=True):
        ...
        count_lowlevel = 0
        data_collected = []
        data_collection_step = 0  # 🔥 新增：实际数据收集步数计数器
        ...
        for step in range(steps * cfg.sim_config.decimation):
            ...
            # 🔥 修改：使用实际数据收集步数来计算command
            if count_lowlevel % cfg.sim_config.decimation == 0:
                current_command = self.get_current_command(
                    data_collection_step,    # 🔥 使用数据收集步数 (0-1000)
                    steps,                   # 🔥 使用实际数据收集总数 (1000)
                    command, 
                    use_gradual_acceleration
                )
                data_collection_step += 1  # 🔥 增加数据收集步数计数器
            else:
                # 如果不是数据收集步，使用上一次的command
                if 'current_command' not in locals():
            ...
            if count_lowlevel % cfg.sim_config.decimation == 0:
                ...
                obs[0, 2] = current_command[0] * cfg.normalization.obs_scales.lin_vel  # cmd.vx
                obs[0, 3] = current_command[1] * cfg.normalization.obs_scales.lin_vel  # cmd.vy
                obs[0, 4] = current_command[2] * cfg.normalization.obs_scales.ang_vel  # cmd.dyaw
                obs[0, 5] = current_command[3]  # cmd.stand
    ...
    def simulate_and_collect_isaac(self, params, steps=1000, command=[2.0, 0., 0., 0.], use_gradual_acceleration=True):
        """使用Isaac Gym环境收集仿真数据"""
        ...
        for step in tqdm(range(steps), desc="Isaac Gym simulation"):
            ...
            # 设置command
            self.env.commands[:, 0] = current_command[0]
            self.env.commands[:, 1] = current_command[1]
            self.env.commands[:, 2] = current_command[2]
            self.env.commands[:, 3] = current_command[3]
    ...
    def objective(self,trial: optuna.Trial) -> float:
        ...
        # 收集Isaac Gym数据
        sim_data = self.simulate_and_collect_isaac(args,steps=500, command=[2.0, 0., 0., 0.])
        distance = self.compute_distance(sim_data, self.real_data)
...
if __name__ == "__main__":
    ...
    #jit_policy_path = f"../logs/kuavo_jog/exported/policies_testnew/policy_1.pt"
    jit_policy_path = f"../logs/kuavo_jog/exported/policies_testnew29000/policy_1.pt"
    #jit_policy_path = f"../logs/kuavo_jog/exported/policies_new2/policy_1.pt"
    #jit_policy_path = f"../logs/kuavo_jog/exported/policies_testold/policy_1.pt"
    #jit_policy_path = f"../logs/kuavo_jog/exported/policies_pretraining/policy_1.pt"
    ...
    real_data = optimizer.collect_mujoco_data(steps=1000, command=[2.0, 0., 0., 0.], use_gradual_acceleration=True)
    ...
    try:
        ...
        # 生成对比图
        optimizer.save_and_visualize_data(
            optimizer.real_data, 
            sim_data, 
            fixed_best_params,
            command=[2.0, 0., 0., 0.],  # 🔥 添加这一行
            use_gradual_acceleration=True
        )
