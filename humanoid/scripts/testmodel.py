...
class Sim2RealCMAOptimizer:
    ...
    def save_and_visualize_data(self, real_data, sim_data, params=None):
        ...
        if hasattr(self, 'last_aligned_sim_data') and hasattr(self, 'last_aligned_real_data'):
            ...
            # 显示详细的对齐信息
            joint_pos_delay = getattr(self, 'last_joint_pos_delay', None)
            joint_vel_delay = getattr(self, 'last_joint_vel_delay', None)
            actions_delay = getattr(self, 'last_actions_delay', None)
            base_vel_delay = getattr(self, 'last_base_vel_delay', None)
            torques_delay = getattr(self, 'last_torques_delay', None)
            
            if all(d is not None for d in [joint_pos_delay, joint_vel_delay, actions_delay, base_vel_delay, torques_delay]):
                print(f"已保存对齐后的数据")
                print(f"  各数据类型时移: pos={joint_pos_delay}, vel={joint_vel_delay}, act={actions_delay}, base={base_vel_delay}, torq={torques_delay}")
            else:
                # 兼容旧版本
                actions_delay_old = getattr(self, 'last_actions_delay', None)
                others_delay_old = getattr(self, 'last_others_delay', None)
                if actions_delay_old is not None and others_delay_old is not None:
                    print(f"已保存对齐后的数据，Actions时移: {actions_delay_old}, Others时移: {others_delay_old}")
                else:
                    print(f"已保存对齐后的数据，时移: {getattr(self, 'last_delay', 'Unknown')}")
...
"""
    ...
    def plot_all_real_data_torque_velocity_curves(self, all_real_data, sim_data=None, filename='all_real_data_torque_velocity_curves.png'):
        ...
        # 获取当前优化参数（用于绘制理论曲线）
        params = getattr(self, 'fixed_best_params', {})
    ...
    def update_velocity_limits(self, env, params):
        ...
        # 更新 dof_vel_limits 张量
        env.dof_vel_limits[2, 0] = params['angle_vel_l3_bottom']  # leg_l3 最小速度
        env.dof_vel_limits[2, 1] = params['angle_vel_l3_top']     # leg_l3 最大速度
        env.dof_vel_limits[8, 0] = params['angle_vel_l3_bottom']  # leg_r3 最小速度
        env.dof_vel_limits[8, 1] = params['angle_vel_l3_top']     # leg_r3 最大速度
        
        env.dof_vel_limits[3, 0] = params['angle_vel_l4_bottom']  # leg_l4 最小速度
        env.dof_vel_limits[3, 1] = params['angle_vel_l4_top']     # leg_l4 最大速度
        env.dof_vel_limits[9, 0] = params['angle_vel_l4_bottom']  # leg_r4 最小速度
        env.dof_vel_limits[9, 1] = params['angle_vel_l4_top']     # leg_r4 最大速度
    ...
    def align_time_series(self, sim_data, real_data, alignment_dims=None):
        ...
        print(f"=== 时间序列对齐开始 ===")
        print(f"Sim data shape: {sim_data.shape}, Real data shape: {real_data.shape}")
        
        # 数据结构：[joint_pos(12), joint_vel(12), action(12), base_lin_vel(3), world_lin_vel(3), actual_torques(12)]
        # 索引：      0-11         12-23         24-35      36-38           39-41            42-53
        
        # 分离不同类型的数据
        joint_pos_sim = sim_data[:, 0:12]      # joint positions
        joint_vel_sim = sim_data[:, 12:24]     # joint velocities  
        actions_sim = sim_data[:, 24:36]       # actions
        base_vel_sim = sim_data[:, 36:39]      # base velocity
        world_vel_sim = sim_data[:, 39:42]     # world velocity
        torques_sim = sim_data[:, 42:54]       # actual torques
        
        joint_pos_real = real_data[:, 0:12]
        joint_vel_real = real_data[:, 12:24]
        actions_real = real_data[:, 24:36]
        base_vel_real = real_data[:, 36:39]
        world_vel_real = real_data[:, 39:42]
        torques_real = real_data[:, 42:54]

        def calculate_alignment_delay(data_sim, data_real, name, dims_to_use=None):
            """计算特定数据类型的对齐延迟"""
            if dims_to_use is None:
                dims_to_use = [0, 1]  # 默认使用前两个维度
            
            delays = []
            for i in dims_to_use:
                if i >= data_sim.shape[1] or i >= data_real.shape[1]:
                    continue
                    
                # 检查数据方差
                if np.var(data_real[:, i]) < 1e-6 or np.var(data_sim[:, i]) < 1e-6:
                    print(f"  {name} dim {i}: 低方差，跳过")
                    continue
                
                # 计算互相关
                correlation = signal.correlate(data_real[:, i], data_sim[:, i], mode='full')
                delay = correlation.argmax() - (len(data_sim) - 1)
                delays.append(delay)
                print(f"  {name} dim {i}: delay = {delay}")
            
            if delays:
                final_delay = int(np.median(delays))
                print(f"  {name} 最终延迟: {final_delay} (基于 {len(delays)} 个维度)")
                return final_delay
            else:
                print(f"  {name} 无有效延迟，使用 0")
                return 0

        # 计算不同数据类型的延迟
        print("\n=== 计算各数据类型的对齐延迟 ===")
        
        # 1. Joint positions (使用 l3, l4 关节，索引 2, 3)
        joint_pos_delay = calculate_alignment_delay(joint_pos_sim, joint_pos_real, "joint_pos", [3])
        
        # 2. Joint velocities (使用相同的 l3, l4 关节)
        joint_vel_delay = calculate_alignment_delay(joint_vel_sim, joint_vel_real, "joint_vel", [2, 3])
        
        # 3. Actions (使用前几个维度)
        actions_delay = calculate_alignment_delay(actions_sim, actions_real, "actions", [0, 1, 2, 3])
        
        # 4. Base velocity (使用 x, y 维度)
        base_vel_delay = calculate_alignment_delay(base_vel_sim, base_vel_real, "base_vel", [0])
        
        # 5. Torques (使用 l3, l4 关节)
        torques_delay = calculate_alignment_delay(torques_sim, torques_real, "torques", [2])

        def apply_alignment(data_sim, data_real, delay, name):
            """应用对齐裁剪"""
            if delay > 0:
                # sim_data 滞后于 real_data
                common_len = min(len(data_real) - delay, len(data_sim))
                if common_len <= 0:
                    print(f"  {name}: 延迟过大，无法对齐")
            ...
            print(f"  {name}: 对齐后长度 {len(aligned_sim)} (延迟: {delay})")
            return aligned_sim, aligned_real

        # 分别对每种数据类型进行对齐
        print("\n=== 应用对齐 ===")
        joint_pos_sim_aligned, joint_pos_real_aligned = apply_alignment(joint_pos_sim, joint_pos_real, joint_pos_delay, "joint_pos")
        joint_vel_sim_aligned, joint_vel_real_aligned = apply_alignment(joint_vel_sim, joint_vel_real, joint_vel_delay, "joint_vel")
        actions_sim_aligned, actions_real_aligned = apply_alignment(actions_sim, actions_real, actions_delay, "actions")
        base_vel_sim_aligned, base_vel_real_aligned = apply_alignment(base_vel_sim, base_vel_real, base_vel_delay, "base_vel")
        world_vel_sim_aligned, world_vel_real_aligned = apply_alignment(world_vel_sim, world_vel_real, base_vel_delay, "world_vel")  # 使用base_vel的延迟
        torques_sim_aligned, torques_real_aligned = apply_alignment(torques_sim, torques_real, torques_delay, "torques")
        
        # 确保所有部分长度一致（取最小长度）
        all_lengths = [
            len(joint_pos_sim_aligned), len(joint_pos_real_aligned),
            len(joint_vel_sim_aligned), len(joint_vel_real_aligned),
            len(actions_sim_aligned), len(actions_real_aligned),
            len(base_vel_sim_aligned), len(base_vel_real_aligned),
            len(world_vel_sim_aligned), len(world_vel_real_aligned),
            len(torques_sim_aligned), len(torques_real_aligned)
        ]
        ...
        # 裁剪到统一长度
        joint_pos_sim_final = joint_pos_sim_aligned[:min_len]
        joint_pos_real_final = joint_pos_real_aligned[:min_len]
        joint_vel_sim_final = joint_vel_sim_aligned[:min_len]
        joint_vel_real_final = joint_vel_real_aligned[:min_len]
        actions_sim_final = actions_sim_aligned[:min_len]
        actions_real_final = actions_real_aligned[:min_len]
        base_vel_sim_final = base_vel_sim_aligned[:min_len]
        base_vel_real_final = base_vel_real_aligned[:min_len]
        world_vel_sim_final = world_vel_sim_aligned[:min_len]
        world_vel_real_final = world_vel_real_aligned[:min_len]
        torques_sim_final = torques_sim_aligned[:min_len]
        torques_real_final = torques_real_aligned[:min_len]
        
        # 重新组合数据
        aligned_sim_data = np.concatenate([
            joint_pos_sim_final,   # 0-11
            joint_vel_sim_final,   # 12-23
            actions_sim_final,     # 24-35
            base_vel_sim_final,    # 36-38
            world_vel_sim_final,   # 39-41
            torques_sim_final      # 42-53
        ], axis=1)
        
        aligned_real_data = np.concatenate([
            joint_pos_real_final,  # 0-11
            joint_vel_real_final,  # 12-23
            actions_real_final,    # 24-35
            base_vel_real_final,   # 36-38
            world_vel_real_final,  # 39-41
            torques_real_final     # 42-53
        ], axis=1)

        # 保存对齐信息
        self.last_aligned_sim_data = aligned_sim_data
        self.last_aligned_real_data = aligned_real_data
        self.last_joint_pos_delay = joint_pos_delay
        self.last_joint_vel_delay = joint_vel_delay
        self.last_actions_delay = actions_delay
        self.last_base_vel_delay = base_vel_delay
        self.last_torques_delay = torques_delay

        print(f"\n=== 对齐完成 ===")
        print(f"各数据类型延迟: pos={joint_pos_delay}, vel={joint_vel_delay}, act={actions_delay}, base={base_vel_delay}, torq={torques_delay}")
        print(f"最终对齐长度: {len(aligned_sim_data)}")
...
if __name__ == "__main__":
    ...
    print(f"=== 使用固定参数运行模型 ===")
    print(f"固定参数: {fixed_best_params}")
    ...
    if has_torque_data:
        ...
        torque_coefficients = np.array([2.0, 1.2, 1.2, 4.1, 2.1, 2.1] * 2)
        torques = torques * torque_coefficients
        print(f"Found and processed torque data with shape: {torques.shape}")
    ...
    try:
        ...
        #     all_real_data, 
        #     sim_data=None,
        #     filename='all_real_data_with_fixed_theory_curves.png'
        # )
        # print("✅ 使用固定参数的理论曲线绘制完成")
    ...
    try:
        # 收集仿真数据
        sim_data = optimizer.simulate_and_collect_isaac(
            fixed_best_params, 
            steps=500, 
            command=[1.8, 0., 0., 0.]
        )
        
        print(f"✅ 仿真数据收集完成，形状: {sim_data.shape}")
        
        # 计算距离（用于评估）
        distance = optimizer.compute_distance(sim_data, optimizer.real_data, verbose=True)
        print(f"固定参数下的距离分数: {distance:.6f}")
        
        # 生成对比图
        optimizer.save_and_visualize_data(optimizer.real_data, sim_data, fixed_best_params)
        
        # 如果有all_real_data，生成完整对比图
        if 'all_real_data' in locals() and all_real_data is not None:
            optimizer.plot_all_real_data_torque_velocity_curves(
                all_real_data, 
                sim_data=sim_data,
                filename='all_real_data_vs_fixed_params_sim_comparison.png'
            )
            print("✅ 完整对比图生成完成")
        
    except Exception as e:
        print(f"❌ 仿真数据收集或对比图生成失败: {e}")
        import traceback
        traceback.print_exc()
    
    print(f"\n=== 程序执行完成 ===")
    print(f"使用的固定参数: {fixed_best_params}")
    print(f"程序执行目录: {optimizer.save_dir}")
